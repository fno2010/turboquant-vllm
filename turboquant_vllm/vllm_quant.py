...
# Deferred registration flag
_registered = False

# Module-level scratch pool (shared across all MoE layers)
_shared_moe_scratch_pool: Any = None

# ── TurboQuantConfig (module-level so cloudpickle can serialize instances) ──
# Inherits from vLLM's QuantizationConfig — required by register_quantization_config.
# Defined at module level (not inside register()) so cloudpickle finds it by name
# rather than capturing a function-closure that drags in torch.ops._Ops on Blackwell.

from vllm.model_executor.layers.quantization.base_config import (
    QuantizationConfig as _VLLM_QuantizationConfig,
)


class TurboQuantConfig(_VLLM_QuantizationConfig):
    ...
    def get_quant_method(self, layer: nn.Module, prefix: str) -> Any | None:
...
# ── TurboQuantOnlineLinearMethod (module-level so cloudpickle can serialize) ──

class TurboQuantOnlineLinearMethod:
